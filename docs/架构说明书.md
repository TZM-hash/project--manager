...
| `ProjectCollaborationService` | 專案協作時間線、權限、分頁與並發衝突處理 |
...
- `ProjectCollaborationRecord`：專案協作時間線記錄。
...
- `ProjectGanttTask` 同時保存里程碑、負責人、單一直接前置工作、實際開始／完成日。
- `ProjectGanttPlan.RowVersion`：以樂觀並發保護整份甘特聚合。
...
- 網頁、列印與 Excel 同步顯示負責人、前置工作、里程碑與實際日期。
- 專案主檔、甘特計畫與協作記錄皆攜帶 Base64 `RowVersion`；衝突時拒絕覆蓋並要求重新載入。
...
## 20. 背景工作與運維監控（2026-07-15）

- `OperationJobs` 持久化全量匯入、全量匯出、專案批量刪除與保養訂單批量刪除；頁面提交後立即跳轉至「工作進度中心」。
- `OperationJobWorker` 每兩秒領取一筆排隊工作，透過 `OperationHandlerDispatcher` 分派處理器；啟動時會將超過十分鐘的遺留執行中工作回復為可處理狀態。
- `OperationJobService` 統一管理排隊、領取、進度、成功與失敗狀態；`RowVersion` 防止背景狀態更新互相覆蓋。
- `OperationFileStore` 只允許 `input`／`output` 區域，並以完整路徑檢查阻止目錄穿越；實體檔案保存於 `App_Data/operations`。
- 一般使用者只能查看與下載自己建立的工作；管理員可查看全部工作。狀態端點回傳最長 2,000 字的錯誤摘要，下載端點再次套用相同權限隔離。
- `/health/live` 僅驗證程序可回應；`/health/ready` 額外驗證資料庫可連線，適合部署平台的存活與就緒探測。
- 管理員「運維監控」整合資料庫連線、SQL Server 最近完整備份、磁碟空間、最近 24 小時例外與 Worker 心跳。
- 未處理例外由 `ExceptionLogMiddleware` 寫入 `App_Data/logs/exceptions-yyyyMMdd.jsonl`，採無 BOM UTF-8 JSONL，之後仍交回 ASP.NET Core 例外處理流程。
- `scripts/quality-gate.ps1` 統一執行 Release 構建、完整測試、全部 JavaScript 語法檢查及 Playwright 用例發現；傳入 `-RunPlaywright` 時執行鍵盤、200% 縮放、顏色對比與互動回歸。

## 21. 專案協作動態與附件（2026-07-15）

- `ProjectCollaborationRecord` 是協作動態時間線，`IsImportant` 提供重要記錄標記；它與 `AuditLog` 是兩個不同資料流。
- `ProjectCollaborationAttachment` 只保存附件中繼資料，檔案由 `ProjectCollaborationAttachmentStore` 保存於非公開的 `App_Data/collaboration-attachments`。
- 附件副檔名與 25 MB 大小限制在儲存服務集中驗證；檔名以 `Path.GetFileName` 清理並以 GUID 前綴避免碰撞與路徑穿越。
- 下載 Handler 先檢查目前使用者是否可存取專案，再確認附件記錄屬於該專案，最後才開啟檔案串流。
- 協作面板提供協作動態、重要徽章與附件連結；操作稽核仍使用獨立的篩選、分頁與稽核時間線。
- 本功能刻意不建立評論資料表、提及解析器或通知佇列。
